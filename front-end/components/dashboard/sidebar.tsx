...
export function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  const { folders, tasks, createFolder, updateFolder, deleteFolder } =
    useTasks();
  ...
  const handleCreateFolder = async (e: React.FormEvent) => {
    ...
    try {
      await createFolder({
        ...
        color: `bg-${
          ["violet", "blue", "green", "red", "yellow", "pink"][
  ...
  const handleDeleteFolder = async (folderId: string) => {
    try {
      ...
      if (folderTasks.length > 0) {
        toast.error(
          "Cannot delete folder with tasks. Move or delete the tasks first."
        );
        return;
      }
            ...
            <div key={folder.id} className="relative">
              {editingFolderId === folder.id ? (
                ...
                >
                  <div className="flex items-center">
                    <span
                      className={`h-3 w-3 rounded-full ${folder.color} mr-3`}
                    ></span>
